...
//printf( "[DisplaySetAttributes 2]");
	...
	switch(mDisplayFormat)
	{
		...
		case MIKIE_PIXEL_FORMAT_32BPP:
			...
			break;
		default:
			//mSystem.gError->Warning("CMikie::SetScreenAttributes() - Unrecognised display format");
...
//			BlowOut();
		...
		case (AUD0VOL&0xff):
			...
			{
				mAUDIO_0_LAST_COUNT=mSystem.gSystemCycleCount;
				mSystem.gNextTimerEvent=mSystem.gSystemCycleCount;
			}
		...
		case (AUD1VOL&0xff):
			...
			{
				mAUDIO_1_LAST_COUNT=mSystem.gSystemCycleCount;
				mSystem.gNextTimerEvent=mSystem.gSystemCycleCount;
			}
		...
		case (AUD2VOL&0xff):
			...
			{
				mAUDIO_2_LAST_COUNT=mSystem.gSystemCycleCount;
				mSystem.gNextTimerEvent=mSystem.gSystemCycleCount;
			}
		...
		case (AUD3VOL&0xff):
			...
			{
				mAUDIO_3_LAST_COUNT=mSystem.gSystemCycleCount;
				mSystem.gNextTimerEvent=mSystem.gSystemCycleCount;
			}
...
// 22/09/06 Fix to championship rally, IRQ not getting cleared with edge triggered system
		...
		case (CPUSLEEP&0xff):
			...
			//
			{
				...
				mSystem.gCPUWakeupTime=mSystem.gSystemCycleCount+cycles_used;
				SetCPUSleep();
				TRACE_MIKIE2("Poke(CPUSLEEP,%02x) wakeup at cycle =%012d",data,mSystem.gCPUWakeupTime);
			}
			break;
		...
		case (Mtest2&0xff):
			// Test registers are unimplemented
			// lets hope no programs use them.
			//mSystem.gError->Warning("CMikie::Poke() - Write to MTEST2");
...
//			TRACE_MIKIE0("Update()");
	...
	if(mSystem.gSystemCycleCount>0xf0000000)
	{
		...
		mSystem.gThrottleNextCycleCheckpoint-=0x80000000;
		mSystem.gAudioLastUpdateCycle-=0x80000000;
		...
		if(mSystem.gCPUWakeupTime)
		{
			mSystem.gCPUWakeupTime-=0x80000000;
			mSystem.gIRQEntryCycle-=0x80000000;
		}
	}
	...
	if(mSystem.gCPUWakeupTime)
	{
		if(mSystem.gSystemCycleCount>=mSystem.gCPUWakeupTime)
		{
			TRACE_MIKIE0("*********************************************************");
			TRACE_MIKIE0("****              CPU SLEEP COMPLETED                ****");
			TRACE_MIKIE0("*********************************************************");
			ClearCPUSleep();
			mSystem.gCPUWakeupTime=0;
		}
		else
		{
			if(mSystem.gCPUWakeupTime>mSystem.gSystemCycleCount) mSystem.gNextTimerEvent=mSystem.gCPUWakeupTime;
		}
	}
...
//						TRACE_MIKIE1("Update() - TIMER 0 Set NextTimerEvent = %012d",mSystem.gNextTimerEvent);
			}
		}
...
//					decval=(mSystem.gSystemCycleCount-mTIM_2_LAST_COUNT)>>divide;
//				}
...
//				if(mTIM_2_LINKING!=7)
//				{
//					tmp=mSystem.gSystemCycleCount+((mTIM_2_CURRENT+1)<<divide);
//					if(tmp<mSystem.gNextTimerEvent)	mSystem.gNextTimerEvent=tmp;
//				}
...
//				{
			...
			{
				mSystem.gNextTimerEvent=tmp;
				TRACE_MIKIE1("Update() - TIMER 4 Set NextTimerEvent = %012d",mSystem.gNextTimerEvent);
			}
//				}
...
//				TRACE_MIKIE1("Update() - mTIM_4_LINKING = %012d",mTIM_4_LINKING);
	...
	if(mTIM_1_ENABLE_COUNT && (mTIM_1_ENABLE_RELOAD || !mTIM_1_TIMER_DONE))
	{
		...
		if(mTIM_1_LINKING!=7)
		{
			...
			{
				mSystem.gNextTimerEvent=tmp;
				TRACE_MIKIE1("Update() - TIMER 1 Set NextTimerEvent = %012d",mSystem.gNextTimerEvent);
			}
		}
...
//				TRACE_MIKIE1("Update() - mTIM_1_LINKING = %012d",mTIM_1_LINKING);
	...
	if(mTIM_3_ENABLE_COUNT && (mTIM_3_ENABLE_RELOAD || !mTIM_3_TIMER_DONE))
	{
		...
		if(mTIM_3_LINKING!=7)
		{
			...
			{
				mSystem.gNextTimerEvent=tmp;
				TRACE_MIKIE1("Update() - TIMER 3 Set NextTimerEvent = %012d",mSystem.gNextTimerEvent);
			}
		}
...
//				TRACE_MIKIE1("Update() - mTIM_3_LINKING = %012d",mTIM_3_LINKING);
	...
	if(mTIM_5_ENABLE_COUNT && (mTIM_5_ENABLE_RELOAD || !mTIM_5_TIMER_DONE))
	{
		...
		if(mTIM_5_LINKING!=7)
		{
			...
			{
				mSystem.gNextTimerEvent=tmp;
				TRACE_MIKIE1("Update() - TIMER 5 Set NextTimerEvent = %012d",mSystem.gNextTimerEvent);
			}
		}
...
//				TRACE_MIKIE1("Update() - mTIM_5_LINKING = %012d",mTIM_5_LINKING);
	...
	if(mTIM_7_ENABLE_COUNT && (mTIM_7_ENABLE_RELOAD || !mTIM_7_TIMER_DONE))
	{
		...
		if(mTIM_7_LINKING!=7)
		{
			...
			{
				mSystem.gNextTimerEvent=tmp;
				TRACE_MIKIE1("Update() - TIMER 7 Set NextTimerEvent = %012d",mSystem.gNextTimerEvent);
			}
		}
...
//				if(mTIM_6_LINKING!=7)
		{
			...
			{
				mSystem.gNextTimerEvent=tmp;
				TRACE_MIKIE1("Update() - TIMER 6 Set NextTimerEvent = %012d",mSystem.gNextTimerEvent);
			}
		}
...
//				sample+=128;
		...
		for(;mSystem.gAudioLastUpdateCycle+HANDY_AUDIO_SAMPLE_PERIOD<mSystem.gSystemCycleCount;mSystem.gAudioLastUpdateCycle+=HANDY_AUDIO_SAMPLE_PERIOD)
		{
			// Output audio sample
			mSystem.gAudioBuffer[mSystem.gAudioBufferPointer++]=(UBYTE)sample;
			...
			mSystem.gAudioBufferPointer%=HANDY_AUDIO_BUFFER_SIZE;
		}
...
//				if(mAUDIO_0_ENABLE_COUNT && !mAUDIO_0_TIMER_DONE && mAUDIO_0_VOLUME && mAUDIO_0_BKUP)
		if(mAUDIO_0_ENABLE_COUNT && (mAUDIO_0_ENABLE_RELOAD || !mAUDIO_0_TIMER_DONE) && mAUDIO_0_VOLUME && mAUDIO_0_BKUP)
		{
			...
			else
			{
				...
				decval=(mSystem.gSystemCycleCount-mAUDIO_0_LAST_COUNT)>>divide;
			}
			...
			if(mAUDIO_0_LINKING!=7)
			{
				...
				{
					mSystem.gNextTimerEvent=tmp;
					TRACE_MIKIE1("Update() - AUDIO 0 Set NextTimerEvent = %012d",mSystem.gNextTimerEvent);
				}
			}
...
//				if(mAUDIO_1_ENABLE_COUNT && !mAUDIO_1_TIMER_DONE && mAUDIO_1_VOLUME && mAUDIO_1_BKUP)
		if(mAUDIO_1_ENABLE_COUNT && (mAUDIO_1_ENABLE_RELOAD || !mAUDIO_1_TIMER_DONE) && mAUDIO_1_VOLUME && mAUDIO_1_BKUP)
		{
			...
			else
			{
				...
				decval=(mSystem.gSystemCycleCount-mAUDIO_1_LAST_COUNT)>>divide;
			}
			...
			if(mAUDIO_1_LINKING!=7)
			{
				...
				{
					mSystem.gNextTimerEvent=tmp;
					TRACE_MIKIE1("Update() - AUDIO 1 Set NextTimerEvent = %012d",mSystem.gNextTimerEvent);
				}
			}
...
//				if(mAUDIO_2_ENABLE_COUNT && !mAUDIO_2_TIMER_DONE && mAUDIO_2_VOLUME && mAUDIO_2_BKUP)
		if(mAUDIO_2_ENABLE_COUNT && (mAUDIO_2_ENABLE_RELOAD || !mAUDIO_2_TIMER_DONE) && mAUDIO_2_VOLUME && mAUDIO_2_BKUP)
		{
			...
			else
			{
				...
				decval=(mSystem.gSystemCycleCount-mAUDIO_2_LAST_COUNT)>>divide;
			}
			...
			if(mAUDIO_2_LINKING!=7)
			{
				...
				{
					mSystem.gNextTimerEvent=tmp;
					TRACE_MIKIE1("Update() - AUDIO 2 Set NextTimerEvent = %012d",mSystem.gNextTimerEvent);
				}
			}
...
//				if(mAUDIO_3_ENABLE_COUNT && !mAUDIO_3_TIMER_DONE && mAUDIO_3_VOLUME && mAUDIO_3_BKUP)
		if(mAUDIO_3_ENABLE_COUNT && (mAUDIO_3_ENABLE_RELOAD || !mAUDIO_3_TIMER_DONE) && mAUDIO_3_VOLUME && mAUDIO_3_BKUP)
		{
			...
			else
			{
				...
				decval=(mSystem.gSystemCycleCount-mAUDIO_3_LAST_COUNT)>>divide;
			}
			...
			if(mAUDIO_3_LINKING!=7)
			{
				...
				{
					mSystem.gNextTimerEvent=tmp;
					TRACE_MIKIE1("Update() - AUDIO 3 Set NextTimerEvent = %012d",mSystem.gNextTimerEvent);
				}
			}
...
//			if(mSystem.gSystemCycleCount==mSystem.gNextTimerEvent) gError->Warning("CMikie::Update() - mSystem.gSystemCycleCount==mSystem.gNextTimerEvent, system lock likely");
//			TRACE_MIKIE1("Update() - NextTimerEvent = %012d",mSystem.gNextTimerEvent);

	// Now all the timer updates are done we can increment the system
	// counter for any work done within the Update() function, mSystem.gSystemCycleCounter
